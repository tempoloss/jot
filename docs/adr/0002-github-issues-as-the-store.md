# 0002 — GitHub Issues as the store
...
mobile app. So the second interface — reading and editing from a phone — comes
...
- **Write access is not `repo.permissions.push`.** That field reports the
  ...
  The real probe is `POST /issues` with an empty body — GitHub evaluates the
  token permission before validating the payload, so `403` means no write and
